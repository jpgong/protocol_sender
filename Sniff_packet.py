...
def just_a_test():
        ...
        for pn in proto_names:
            if pn in packet:
                packet_proto = pn
                break
        src = packet[packet_proto].src
        dst = packet[packet_proto].dst
        proto = packet_proto
        """
        packet = IP() / TCP(dport=80)
        packet_time = timestamp2time(packet.time)
...
def start_capture():
    # 开启一个新线程，执行sniff开始抓包，对sniff的prn写一个函数，
    t = threading.Thread(sniff(count= 20, prn=(lambda x : just_a_test(x))))
    t.setDaemon(True)
    t.start()
